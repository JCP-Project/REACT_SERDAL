...
import DatasetPageList from "./DatasetPageList";
      ...
      <div className={`px-0 mt-5 ${adminStatus ? 'lg:px-10' : 'lg:px-40'}`}>
        ...
        {loading ? (
          ...
          errorMessage ? (
            ...
            <DatasetPageList dataSets={dataSetGroup} fetchDataSets={fetchDataSets} /> // Display dataset list if no error
          )
        )}
        ...
        <div className="bg-white p-6 rounded-lg max-w-2xl w-full mx-5 px-10 text-lg">
          ...
          <form onSubmit={save} className="w-full">
            ...
            <section className="my-5 ml-1">
              <label
                htmlFor="fileInput"
                className="flex items-center gap-3 px-4 py-2 border border-dashed border-gray-300 rounded-md cursor-pointer hover:bg-gray-50 transition-colors"
              >
