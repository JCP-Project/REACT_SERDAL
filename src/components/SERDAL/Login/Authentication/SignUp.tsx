    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
          const response = await fetch(`${apiUrl}/api/Users/sendOTP`, {
            ...
            body: JSON.stringify(otpData), // Convert the JavaScript object to a JSON string
          });
    ...
    const OTPVerify = (otp: string) => {
      Swal.fire({
        ...
        preConfirm: async (inputOTP) => {
          ...
          const normalizedOtp = otp ? String(otp).trim() : ""; // Convert to string and trim
          const normalizedInputOTP = String(inputOTP).trim();
          console.log("OTP:", normalizedOtp);
          console.log("input:", normalizedInputOTP);
    
          // Compare both values after normalization
          if (normalizedOtp === normalizedInputOTP) {
            // OTP is correct, proceed with your request
            try {
              const data = {
                ID: 0,
                FirstName: formData.firstname,
              ...
              console.log("success2", responseData.id);
